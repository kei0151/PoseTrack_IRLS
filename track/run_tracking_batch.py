import sys
import os
# aic_cpp/ directory in track/ shadows the .so as a namespace package; insert explicitly
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'aic_cpp'))
import argparse
...
def main():
    ...
    cams = sorted([c for c in os.listdir(cal_dir) if c.startswith("camera_")])
    cals = []
    for cam in cams:
    ...
    files = [f for f in files if f.startswith('camera_')]
    for f in files:
