...
def myiou(atracks,btracks):
    ious = np.zeros((len(atracks), len(btracks)), dtype=float)
    if ious.size == 0:
        return ious
    ...
    bbox_p=np.ascontiguousarray(altrbs, dtype=float)
    bbox_g=np.ascontiguousarray(bltrbs, dtype=float)
...
def Giou_np(bbox_p, bbox_g):
    bbox_p=np.ascontiguousarray(bbox_p, dtype=float)
    bbox_g=np.ascontiguousarray(bbox_g, dtype=float)
...
def bboxes_ciou(tracks1,tracks2):
    ...
    boxes1=np.ascontiguousarray(ltrbs1, dtype=float)
    boxes2=np.ascontiguousarray(ltrbs2, dtype=float)
